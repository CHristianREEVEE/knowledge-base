...
## 2026-09-10 | agent 世界架构：公开只读 + 密钥只挡写

- 引擎只读端点零鉴权实测：/api/world、/api/logs、/api/agent/:id 全公开——外人看真实世界动态不需要任何密钥；/api/state、/api/agents 等管理接口才要 adminKey
- 架构启示：给外人做展示/围观入口，直连只读公开端点用真实数据（实时日志流 8s 轮询），不复制数据不做模拟演示——真实就是最好的展示
- 配合密钥红线（09-03）：密钥绝不进 URL 参数，密钥墙只挡管理动作

**#agent世界 #只读端点 #权限设计 #零鉴权**
